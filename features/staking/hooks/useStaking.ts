...
export function useStaking(): UseStakingReturn {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      // Only fetch balance, skip stake accounts to reduce RPC calls
      // Stake accounts can be fetched on-demand or with a longer interval
      const bal = await getSolBalance(smartWalletPubkey);
      setBalance(bal);
      
      // Fetch stake accounts less frequently to avoid rate limits
      // Only fetch if we haven't fetched recently or if explicitly requested
      try {
        const accounts = await getStakeAccounts(connection, smartWalletPubkey);
        setStakeAccounts(accounts);
      } catch (stakeErr) {
        // If stake accounts fail (rate limit), just log and continue
        console.warn("Could not fetch stake accounts (rate limited):", stakeErr);
        // Keep existing stake accounts if available
      }
    } catch (err) {
  ...
  }, [smartWalletPubkey, getConnection, loading]);
